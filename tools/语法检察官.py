...
_LAW = r"""
{
  ...
  "交付形态": "一节切片交付两条视频：知识篇（开场+知识点正文+例题）与刷题篇（课前测+随堂练习）。课程总数仍为 180 节不变。"
}
"""
# LAW_END
...
def check(path):
    ...
    # ── 17 时长：一节切片交付两条视频，分篇各自受限 ──
    wpm = TH.get("口播每分钟字数", 250)
    secs = re.split(r"\*{0,2}(课前测第\d题|例题\d|随堂练习题\d)\*{0,2}", t)
    nm = [secs[k] for k in range(1, len(secs), 2)]
    bd = [secs[k + 1] for k in range(1, len(secs), 2)]
    pre = sum(zh(b) for n_, b in zip(nm, bd) if "课前测" in n_)
    hw = sum(zh(b) for n_, b in zip(nm, bd) if "随堂" in n_)
    b6 = [b for n_, b in zip(nm, bd) if n_ == "课前测第6题"]
    if b6 and "六道课前测结束" in b6[0]:
        c = b6[0].index("六道课前测结束")
        pre -= zh(b6[0][c:])
    drill = pre + hw
    know = total_zh - drill
    km, dm = know / wpm, drill / wpm
    kh = TH.get("知识篇上限分钟", 16)
    dh = TH.get("刷题篇上限分钟", 16)
    over = []
    if km > kh:
        over.append(f"知识篇 {km:.0f} 分（上限 {kh}）")
    if dm > dh:
        over.append(f"刷题篇 {dm:.0f} 分（上限 {dh}）")
    if over:
        issues.append(f"【时长超标】{'；'.join(over)}　全片 {total_zh} 字约 {(km+dm):.0f} 分"
                      f"\n            一节切片交付两条视频："
                      f"知识篇＝开场＋正文＋例题，刷题篇＝课前测＋随堂练习。"
                      f"超了要砍重复例子、旁支知识点、讲过两遍的道理")
    else:
        passes.append(f"时长：知识篇 {km:.0f} 分 + 刷题篇 {dm:.0f} 分")
