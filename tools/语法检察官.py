...
def check(path):
    ...
    # ── 17 时长超标：这是「切片」，一节只讲一个小颗粒 ──
    mins = total_zh / TH.get("口播每分钟字数", 250)
    lo, hi = TH.get("时长下限分钟", 12), TH.get("时长上限分钟", 20)
    if mins > hi:
        issues.append(f"【时长超标】全文 {total_zh} 字，录出来约 {mins:.0f} 分钟，"
                      f"超过 {hi} 分钟上限\n            "
                      f"这是语法「切片」，一节只讲一个小颗粒。"
                      f"要砍：重复的例子、可以并到别节的旁支、讲了两遍的道理")
    elif mins < lo:
        warns.append(f"时长偏短：约 {mins:.0f} 分钟，低于 {lo} 分钟，讲解可能不够透")
    else:
        passes.append(f"时长：约 {mins:.0f} 分钟")
